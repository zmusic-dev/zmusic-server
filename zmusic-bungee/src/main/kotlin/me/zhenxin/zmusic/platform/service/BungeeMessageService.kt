...
class BungeeMessageService : MessageService {
    ...
    override fun broadcast(message: String) {
        ProxyServer.getInstance().broadcast(TextComponent(message.colored()))
    }

    override fun broadcast(message: Component) {
        val legacy = serializer.serialize(message)
        ProxyServer.getInstance().broadcast(TextComponent(legacy))
    }

    override fun broadcastActionBar(message: Component) {
        val legacy = serializer.serialize(message)
        ProxyServer.getInstance().players.forEach {
            it.sendMessage(ChatMessageType.ACTION_BAR, TextComponent(legacy))
        }
    }
}
