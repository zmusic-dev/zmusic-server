...
`zmusic.use` ZMusic のルートコマンドを使用可能

`zmusic.play` 音楽を再生可能

`zmusic.search` 音楽を検索可能

`zmusic.stop` 再生を停止可能

`zmusic.volume` 音量を調整可能

`zmusic.lyric` 歌詞を表示可能

`zmusic.playlist` プレイリストを管理可能

## 管理者権限 {#admin-permissions}

`zmusic.reload` 設定ファイルを再読み込み可能

`zmusic.admin` すべての管理者コマンドを使用可能
